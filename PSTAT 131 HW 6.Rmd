...
```
The best-performing pruned decision tree on the folds has mean ROC_AUC of 0.7264499. 
...
```{r}
rand_forest_grid = grid_regular(mtry(range = c(1, 8)), trees(range = c(1, 3)), 
                                min_n(range = c(1, 50)), levels = 8)
```
